...
@Controller
@RequestMapping("/gameplay/front/single")
public class SingleStartFrontController {
    @Autowired
    ...
    @PostMapping
    public String startSingleGame(Model model, @RequestParam Long playerKey, @RequestParam Long playerId) {
        PlayGroundEntity playGroundEntity;
        try {
            playGroundEntity = gameService.startSinglePlayerGame(playerKey);
